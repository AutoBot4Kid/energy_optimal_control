...
option = optimoptions('fsolve','Display','iter','MaxIterations',10^6,'MaxFunctionEvaluations',10^6);
alphaguess = deg2rad(40);
[alphaV0] = fsolve(@v0fun,alphaguess,option);


fminconOption =  optimoptions('fmincon','Display','iter','Algorithm','sqp','MaxIterations',10^6,'MaxFunctionEvaluations',10^6);
param = [0.1,0.99];
alphavec = linspace (0,alphaV0,31);
RFlb = [0,0];
RFub = [inf,1];
for i = 1 : length(alphavec);
    [ocpParam] = fmincon(@jacobiOCP,param,[],[],[],[],RFlb,RFub,@noncon,fminconOption,alphavec(i));
    param = ocpParam;
...
function [v0] = v0fun(alpha);
fminconOption =  optimoptions('fmincon','Display','iter','Algorithm','sqp','MaxIterations',10^6,'MaxFunctionEvaluations',10^6);
param = [0.1,0.99];
alphavec = linspace (0,alpha,31);
RFlb = [0,0];
RFub = [inf,1];
for i = 1 : length(alphavec);
    [ocpParam] = fmincon(@jacobiOCP,param,[],[],[],[],RFlb,RFub,@noncon,fminconOption,alphavec(i));
